defmodule Explorer.Validator.MetadataImporterTest do
  ...
  describe "import_metadata/1" do
    test "inserts new address names when there's none for the validators" do
      ...
      [%{address_hash: address.hash, name: "Testit Unitorus", metadata: %{"test" => "toast"}}]
      |> MetadataImporter()

      address_names = from(an in Address.Name, where: address_hash == ^address.hash and primary: true) 
      |> Repo.all()
